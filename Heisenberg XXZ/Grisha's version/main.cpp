...
state_t translate(state_t state, index_t translation)
{
	...
	u64 window = (u64(state) << N) | state;
	return (window >> translation) & mask;
}

std::pair<state_t, index_t> getRepresentative(state_t state)
{
	constexpr state_t mask = (state_t(1) << N) - 1;
	u64 window = (u64(state) << N) | state;
...
i32 main()
{
	...
	{
		auto [representative, translation] = getRepresentative(state);
	...
	// std::unordered_map<state_t, index_t> indices; for(index_t i = 0; i < basis.size(); i++) indices[basis[i]] = i;
	...
	for(index_t stateIndex = 0; stateIndex < basis.size(); stateIndex++)
	{
		...
		for(auto bond: bonds)
		{
			...
			auto [representative, translation] = getRepresentative(newState); // Can be hashed, but will be slower
